...
통계를 제공하는 백엔드 채용 과제입니다. Apple Health 요청의 `data.source.name`은 `Health Kit`입니다.
...
`from`과 `to`가 반환 항목 수를 결정합니다.

## 조회 결과

Docker Compose로 실행한 컨테이너에서 확인한 `recordkey`별 Daily·Monthly 전체 결과는 별도 제출물인
...
|---|---|---|---|---|
| `steps` | 걸음수 측정값(응답은 int) | JSON 정수 | `DECIMAL(24,12)` | 입력의 소수 걸음수를 보존해 합산한 뒤 정수로 반올림 |
| `calories` | 소모 칼로리(float) | JSON 소수 | `DECIMAL(24,12)` | 이진 부동소수점의 누적 오차 없이 합산 |
| `distance` | 이동거리(float) | JSON 소수 | `DECIMAL(24,12)` | 소수점 열두 자리까지 보존한 뒤 응답에서 여섯 자리로 반올림 |
...
2월 29일로 보정됩니다. 각 측정 시작 시각을 `Asia/Seoul` 날짜로 변환해 집계 기준 날짜로 저장했습니다.

### 3. 같은 회원이 같은 recordkey로 동시에 저장하면 500이 났습니다

두 요청이 같은 연결의 활동 레코드를 동시에 조회하면 양쪽이 같은 활동을 신규로 분류할 수 있습니다.
이때 나중에 저장하는 요청이 UNIQUE 제약조건을 위반해 500 응답을 받았습니다. **Docker 환경의 동시
요청 검증에서 발견했고 자동 테스트로 재현했습니다.**

활동 레코드를 조회하기 전에 연결 행에 `PESSIMISTIC_WRITE` 데이터베이스 쓰기 잠금을 걸어 같은 연결의
저장을 순서대로 처리했습니다. 활동 레코드 저장을 재시도로 처리하지 않은 이유는 충분한 재시도 횟수를
정할 근거가 없기 때문입니다.

### 4. 일간 집계 결과를 합산하면 월간 집계에 반올림 오차가 누적됐습니다

명세는 집계를 마친 뒤 반올림하도록 요구합니다. 반올림된 일간 결과를 다시 합산해 월간 결과를 만들면
**회귀 기준 8건 중 5건이 달라집니다.** 걸음수는 1, 소수 값은 소수점 여섯째 자리에서
0.000001~0.000002의 차이가 발생합니다.

월간 조회는 데이터베이스에 저장된 반올림 전 측정값을 월 단위로 직접 합산하고, 최종 응답에서 한 번만
반올림합니다. 통합 테스트로 월간 회귀 기준과의 일치 여부를 검증합니다.

### 5. 테스트가 통과해도 일부 계약을 검증하지 못했습니다

- **응답 소수 자리**: JSON 트리로 읽으면 Jackson이 `0.000000`을 `0.0`으로, `BigDecimal`로 바꿔도
  소수부 뒤의 0이 제거됩니다. 전송되는 문자열 자체가 계약이므로 **응답 원문**을 비교하도록
  바꿨습니다
- **실행 계획**: 사용 가능한 인덱스 목록인 `possible_keys`만 확인해 테이블 전체 조회를 선택해도
  단언이 통과했습니다. 실제 선택한 인덱스인 `key`와 사용한 인덱스 열인 `used_key_parts`를 확인하도록
  바꿨습니다. 대량 삽입 직후에는 `ANALYZE TABLE`로 통계를 갱신합니다
- **회귀 기준 순서**: `Map.copyOf`가 JVM 실행마다 순회 순서를 무작위화해, 첫 항목을 쓰는 테스트가
  실행마다 다른 `recordkey`를 골랐습니다. 순서 보존 불변 뷰로 바꿨습니다

각 테스트가 관련 결함을 실제로 검출하는지는 구현을 의도적으로 변경했을 때 실패하는지 확인했습니다.
...
기본 로그아웃 설정을 비활성화하고 `UserDetailsService` 자동 구성을 제외했습니다. 제외를 애노테이션이
아니라 설정 속성으로 둔 이유는 애노테이션 제외가 `@WebMvcTest`로 구성한 MVC 테스트에 전파되지 않기
때문입니다.
...
**전체 247건.** Docker 환경에서 이미지 빌드부터 로그아웃까지 9단계를 확인했습니다.
