...
- Spring REST Docs를 사용해 테스트로 검증된 API 문서를 생성한다.
...
| API Docs | Spring REST Docs | 테스트 기반 API 계약 문서 |
...
[`과제_원문.md`](../요구사항/과제_원문.md)의 제출물 1번과 2번이 소스코드와 ERD에 코멘트를 요구하므로 주석은 평가 대상이다.

- 모든 주석은 한국어로 작성한다.
- 공개 클래스와 도메인 메서드에는 Javadoc으로 목적과 정책 근거를 남긴다.
- 코드로 이미 드러나는 내용을 반복하지 않고, 공급자별 정규화·멱등성·소유권처럼 근거가 필요한 결정을 설명한다.
- 정책을 설명할 때는 근거 문서의 위치를 함께 적는다.
- 테스트 주석은 검증 의도를 설명한다.
- 테이블과 컬럼에는 한국어 `COMMENT`를 남긴다.
...
- 원본 측정값은 `BigDecimal`과 `DECIMAL(24, 12)`로 보존한다.
...
- 적용된 migration은 수정하지 않고 새로운 보정 migration으로 전진 처리한다.
...
- [ ] 필수 설정 누락과 안전하지 않은 JWT secret으로 기동되지 않는다.
- [ ] Flyway가 빈 MySQL에 전체 스키마를 생성한다.
- [ ] 적용된 migration을 수정하지 않고 전진 보정할 수 있다.
- [ ] MySQL UNIQUE 제약조건으로 멱등성을 최종 보장한다.
- [ ] Redis 리프레시 토큰의 TTL, 회전과 폐기가 동작한다.
- [ ] 집계 캐시와 version 무효화가 동작한다.
- [ ] Redis 장애 시 MySQL 집계 조회가 성공한다.
- [ ] 전체 자동 테스트가 통과한다.
- [ ] Docker Compose에서 세 서비스가 healthy 상태가 된다.
- [ ] 실제 API 검증 절차와 결과가 문서화된다.
- [ ] 기능 완료 조건은 [`기능_명세.md`](../명세/기능_명세.md)의 계약과 회귀 기준을 모두 만족한다.
