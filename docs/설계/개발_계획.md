...
- MySQL을 회원·건강활동 데이터의 영속 원본으로, Redis를 리프레시 토큰 저장과 회전에 사용합니다.
- Flyway와 Testcontainers로 스키마 변경과 MySQL·리프레시 토큰용 Redis 통합 동작을 검증합니다.
...
| Token Store | Redis 7 (`redis:7-alpine`) | 리프레시 토큰 저장과 회전 |
...
- Controller는 HTTP 계약, 인증된 회원 ID 확인, 입력 검증과 응답 변환을 담당합니다.
- Service는 회원가입, 인증, 정규화, 소유권과 멱등 저장을 담당합니다.
...
[`과제_원문.md`](../요구사항/과제_원문.md)의 제출물 1번과 2번이 소스코드와 ERD에 코멘트를 요구하므로
주석은 평가 대상입니다. 제출물 요구를 충족하는 범위에서 필요한 주석만 작성합니다.
...
- `health_connections.record_key`는 `VARCHAR(255)`으로 저장하며 대소문자와 악센트를 구분해 비교합니다.
...
- 정수부가 컬럼 범위를 넘는 값은 정규화 단계에서 `400 Bad Request`로 거부해 DB 범위 오류가
  `500 Internal Server Error`로 노출되지 않게 합니다.
...
- 적용된 migration은 수정하지 않고 새 migration을 추가해 변경합니다. 주석도 checksum에 포함되므로
  주석만 고쳐도 기존 데이터베이스의 validation이 실패합니다. 이 때문에 적용된 migration은
  「주석 원칙」과 「코드 서식」의 적용 대상에서 제외합니다.
...
- `recordkey` 연결 해석과 활동 레코드 저장은 별도 트랜잭션으로 나누고, 동시 연결 생성의 UNIQUE 충돌은
  새 트랜잭션에서 해당 `recordkey`의 연결을 한 번 다시 조회합니다. 트랜잭션을 분리하는 메서드는
  별도 빈으로 둡니다.
- 활동 레코드 저장은 연결 행을 잠근 뒤 500건 단위로 처리합니다. 응답 카운트는 애플리케이션에서
...
Redis는 리프레시 토큰 저장과 회전에만 사용합니다. 연결 대기는 1초, 명령 응답 대기는 2초로
제한하며, 처리를 완료하지 못하면 리프레시 토큰 작업을 실패시킵니다.
...
- Compose는 `app`, `mysql`, `redis`의 healthcheck가 모두 `healthy`인지 확인합니다.
- `/actuator/health`는 인증 없이 접근할 수 있고, 정상 상태에서 종합 결과 `{"status":"UP"}`만
  반환합니다.
...
- MySQL과 Redis에 healthcheck를 설정하고, `app`은 Actuator healthcheck로 확인합니다.
- `app`은 의존 서비스가 healthy 상태가 된 후 시작합니다.
- MySQL 데이터는 named volume에 보존합니다.
- Redis는 AOF를 사용합니다.
- `.env.example`만 저장소에 포함하고 실제 `.env`는 제외합니다.
- `/actuator/health`는 MySQL과 Redis 상태를 반영한 종합 결과만 반환합니다.
...
변경 코드와 가장 가까운 테스트부터 실행하고 Unit, Slice, MySQL·리프레시 토큰용 Redis Integration,
E2E, 전체 빌드, Docker API smoke 순서로 검증 범위를 넓힙니다.
...
- MySQL에서 실행되는 쿼리, FK, UNIQUE 제약조건과 Daily/Monthly 직접 집계
- 리프레시 토큰 TTL, 회전과 폐기
- 동일 입력 재전송과 값 변경
- `recordkey` 소유권 충돌
...
[`../../fixtures/health`](../../fixtures/health)의 `INPUT_DATA1.json`부터 `INPUT_DATA4.json`까지
사용합니다. 입력 특성과 Daily/Monthly 기대값은
[`기능_명세.md`](../명세/기능_명세.md)의 회귀 기준을 단일 기준으로 사용합니다.
...
4. 네 입력 파일 저장
5. 같은 입력 파일 재전송
6. Daily/Monthly API 조회와 MySQL 집계 결과 대조
7. 리프레시 토큰 재발급과 회전
8. 로그아웃 후 리프레시 토큰 거절
9. 로그와 비밀정보 점검
...
| 4 | `feat: 회원가입과 인증 구현` | 회원가입, 로그인, JWT, Redis 리프레시 토큰, 오류 응답 | Unit·MVC·MySQL·리프레시 토큰용 Redis 테스트 |
| 5 | `feat: 건강활동 데이터 정규화와 저장` | 공급자 파싱, UTC·숫자 정규화, 소유권, 멱등 저장 | 네 입력 파일 저장·재전송·변경·권한 테스트 |
| 6 | `feat: 일간·월간 집계 조회 구현` | DB 집계와 응답 계약 | 회귀값과 경계 날짜 테스트 |
| 7 | `test: 통합 및 회귀 테스트 보강` | Testcontainers와 전체 사용자 흐름 E2E 보강 | 전체 테스트와 빌드 |
| 8 | `docs: ERD·API 예시와 검증 결과 작성` | README, ERD, API, 실제 Docker 검증 결과 | Compose, API smoke와 링크 점검 |
...
- [x] 적용된 migration을 수정하지 않고 새 migration으로 변경할 수 있습니다.
- [x] MySQL UNIQUE 제약조건으로 멱등성을 최종 보장합니다.
- [x] Redis 리프레시 토큰의 TTL, 회전과 폐기가 동작합니다.
- [x] Daily·Monthly 집계가 MySQL 원본을 직접 조회합니다.
- [x] 전체 자동 테스트가 통과합니다.
