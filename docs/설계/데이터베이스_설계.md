...
- [`V3__record_key_varchar.sql`](../../src/main/resources/db/migration/V3__record_key_varchar.sql)

컬럼 타입과 제약조건이 이 문서와 migration에서 다르면 실제 적용 기준인 migration을 따릅니다. 각
선택의 이유는 [`개발_계획.md`](개발_계획.md)에 있습니다.
...
- 엔진은 `InnoDB`, 문자셋은 `utf8mb4`입니다. 기본 콜레이션은 `utf8mb4_unicode_ci`이며,
  `members.email`과 `health_connections.record_key`는 대소문자와 악센트를 구분하는
  `utf8mb4_0900_as_cs`를 사용합니다.
...
| `record_key` | `VARCHAR(255)` UK | 공급자가 전달한 사용자 구분 키(recordkey). 대소문자·악센트 구분 비교 |
...
`record_key`는 `VARCHAR(255)`으로 저장하고 대소문자와 악센트를 구분해 비교합니다. UNIQUE로 두어
**recordkey 하나가 회원 한 명에게만 귀속**됩니다. 최초 저장 요청을 보낸 회원이 소유자가 됩니다.
...
| `payload_hash` | `CHAR(64)` | 측정값 변경 감지를 위한 SHA-256 해시(hex). 원본 요청 본문은 저장하지 않습니다 |
| `created_at` | `DATETIME(6)` | 생성 시각 (UTC) |
| `updated_at` | `DATETIME(6)` | 수정 시각 (UTC) |
...
| **`ix_health_activity_records_connection_date`** | `(connection_id, activity_date)` | **Daily/Monthly 집계가 사용하는 기본 인덱스** |
| `ix_health_activity_records_connection_period` | `(connection_id, period_start_utc, period_end_utc)` | 기간 범위 조회. 멱등성 UNIQUE 인덱스는 `metric_type`이 두 번째 열이어서 기간 조건만으로 집계 조회에 사용할 수 없습니다 |
